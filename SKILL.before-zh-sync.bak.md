...
Do not edit this skill itself unless the user explicitly asks to modify, update, or fix the skill. If the user gives a one-time preference for the current deck, apply it only to the current task and do not persist it into `SKILL.md`.
...
5. Generate each slide as a separate full-page 1920x1080 image with `gpt-image-2`, and ensure the final page image contains accurate audience-facing Chinese text.
...
Use calm, constructive, participant-respecting language in all slide text and speaker notes.

- Prefer defining the goal, showing the next action, or explaining the benefit directly.
- Avoid repetitive oppositional or disruptive slogan patterns such as "不是……而是……", "别再……", and claims that dismiss existing practice.
- Describe an addition, improvement, or applicable condition without framing the learner's current method as wrong or obsolete.
- Keep questions exploratory and practical; invite learners to contribute their domain knowledge instead of prescribing a conclusion.
...
The final deliverable for every slide is a complete audience-facing picture slide with readable, accurate Chinese text already baked into the exported PNG. Do not deliver or present textless background images as finished slides.

Preferred exact-text workflow:

- Generate a visual page/background with `gpt-image-2`; avoid asking the image model to invent or render complex Chinese text when fidelity matters.
- Add the exact Chinese title, labels, numbers, and short phrases as a separate layout/text layer, then export a single full-slide PNG for the final picture-based PPT.
- Keep a matching textless source image and layout metadata for every slide. If the user later asks to "remove text" or "去掉文字", export the same slide visual without the text layer so all generated visual content, framing, colors, and composition stay unchanged.
- Do not regenerate, crop, mask, or redesign the image when the user only asks to remove text.
...
- If a textless background is used for title fidelity, add all required audience-facing slide text into the final PNG before quality inspection. Never treat a background-only image as a completed courseware page.
...
Do not use Python, python-pptx, or other Python-based tooling to create or assemble the PowerPoint deck.
